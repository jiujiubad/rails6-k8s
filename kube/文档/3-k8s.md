```
cd kube
kubectl label nodes docker-desktop projectType=web projectName=rails6 
make test
```
...
注释掉 affinity，以及所有 `imagePullPolicy: Always` 以加快本地测试速度，要注释的文件有
...
执行，当 puma pod 为 READY 1/1 时，打开 <http://localhost:30003>。调试的命令，可查看 `kube/文档/4-命令行替代dashboard.md`
...
- [sportngin/okcomputer 更新至 2020 年 Rails6](https://github.com/sportngin/okcomputer)
...
修改 Makefile 最上面的变量，在 test 方法追加代码。中间的 `cat Gemfile` 命令用于检查已修改的代码有无更新，根据已修改的文件修改

还可检查镜像中有无包含私钥文件如 .env、kube/env/secret*.yaml、config/application.yml

```
test:
	docker build -t $(IMAGE_NAME):$(tag) ..
	docker run --rm $(IMAGE_NAME):$(tag) cat Gemfile
...
> 注意：为确保每次代码都能被更新到镜像中，每次 make update 时 tag 都要使用不同的版本号。首次推送即 make first 则是使用 --no-cache 参数确保镜像能完全重建，但构建速度会慢很多，且每次重新下载环境依赖也比较浪费流量
...
```
make first                  #第一次执行，build和push标签为1.0.0的镜像，并执行apply
...
***报错：ActiveSupport::MessageEncryptor::InvalidMessage ArgumentError: key must be 16 bytes***